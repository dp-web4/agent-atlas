...
**Read: 41 coding-agent harnesses** (from ctx). **Talk-to: 41 descriptors**, one per
harness directory; each carries a `fidelity` mark in its frontmatter (`verified` for
`claude` and `kimi_code_cli`, which run against live hestia adapters; `documented` /
`inferred` for the rest, sourced from vendor docs). Improving a descriptor — or
raising its fidelity by wiring it against the real harness — is a documentation task
against [`../talk-to/SCHEMA.md`](../talk-to/SCHEMA.md), not a code port.
